...
class DocumentacionApp:
    ...
    def _analizar_funciones_con_ia(self, funciones):
        ...
        for funcion in funciones:
            ...
            prompt = f"""Eres un experto en documentación de código Python. Genera un docstring claro y profesional.

CÓDIGO DE LA FUNCIÓN:
{codigo}

INSTRUCCIONES:
1. Describe QUÉ hace la función de forma clara
2. Especifica los tipos de datos de parámetros
3. Indica qué retorna la función
4. Genera SOLO el contenido del docstring (sin triples comillas)
5. NO incluyas "def" ni código
6. Máximo 15 líneas

FORMATO ESTÁNDAR:
Descripción clara de la función en 1-2 líneas.

Args:
    parametro1 (tipo): Descripción del parámetro y su propósito.
    parametro2 (tipo): Descripción del parámetro y su propósito.

Returns:
    tipo: Descripción del valor retornado.

Responde SOLO con el docstring formateado."""
    ...
    def _limpiar_docstring(self, texto):
        """Limpia el texto del docstring removiendo formato markdown, repeticiones y extras."""
        ...
        # Detectar y eliminar repeticiones y texto meta
        texto_visto = set()
        saltar_resto = False
        ...
        for linea in lineas:
            ...
            # Saltar líneas con "def "
            if linea_limpia.startswith('def '):
                continue
            
            # Saltar líneas meta del modelo
            if any(frase in linea_limpia.lower() for frase in [
                'función:',
                'en cuanto a',
                'aquí hay',
                'posible forma',
            ...
            # Si encontramos texto repetido, detener
            if saltar_resto:
                continue
            
            # Detectar repetición por hash de línea
            if linea_limpia and len(linea_limpia) > 20:  # Solo líneas significativas
                if linea_limpia in texto_visto:
                    # Encontramos repetición, cortar aquí
                    break
                texto_visto.add(linea_limpia)
            
            lineas_limpias.append(linea)
        
        texto = '\n'.join(lineas_limpias).strip()
        
        # Remover líneas vacías consecutivas excesivas
        while '\n\n\n' in texto:
            texto = texto.replace('\n\n\n', '\n\n')
    ...
    def regenerar_sugerencia(self):
        ...
        # Prompt mejorado para regeneración - Versión MÁS DETALLADA
        prompt = f"""Eres un experto en documentación técnica de Python. Analiza esta función y genera un docstring MÁS COMPLETO Y DETALLADO.

CÓDIGO DE LA FUNCIÓN:
{codigo_funcion}

DOCSTRING ANTERIOR (SIMPLE):
{sugerencia_actual['docstring'][:200]}

TU TAREA: Genera una versión MEJORADA con MÁS DETALLES:
- Describe el PROPÓSITO y el CONTEXTO de uso
- Especifica TIPOS DE DATOS de forma precisa
- Menciona CASOS ESPECIALES o COMPORTAMIENTOS importantes
- Agrega información sobre VALIDACIONES o RESTRICCIONES si aplica
- Incluye EJEMPLOS de uso si es apropiado
- Menciona EXCEPCIONES que podría lanzar si es relevante

FORMATO REQUERIDO:
Descripción detallada del propósito de la función y su comportamiento.
Puede incluir contexto de cuándo usarla.

Args:
    parametro1 (tipo exacto): Descripción detallada del parámetro,
        incluyendo restricciones, rangos válidos o valores especiales.
    parametro2 (tipo exacto): Descripción completa con ejemplos si ayuda.

Returns:
    tipo exacto: Descripción detallada del valor retornado,
        incluyendo formato, rango o casos especiales.

Raises:
    TipoError: Condición que causa esta excepción (si aplica).
...
Notes:
    ...
    - Consideraciones de performance si es importante

IMPORTANTE: 
- NO copies el docstring anterior
- SÉ MÁS ESPECÍFICO Y TÉCNICO
- Máximo 20 líneas
- NO incluyas código ni "def"
- Responde SOLO el docstring"""

        try:
            self.editor.delete('1.0', tk.END)
            self.editor.insert(tk.END, f"🔄 Regenerando docstring MEJORADO para {funcion_actual}...\n")
            self.editor.insert(tk.END, "💡 Generando versión más detallada y específica...\n")
            self.root.update()
